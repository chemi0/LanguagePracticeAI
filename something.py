...
class ChatApp:
    def __init__(self, root, show_language_selection, initial_role, initial_language):
        self.root = root
        ...
        self.role_dropdown = ttk.OptionMenu(root, self.role_var, initial_role, *roles.keys(),
        ...
        self.language_dropdown.grid(row=4, column=0, columnspan=3, pady=5)

        # Back to Language Selection button # Removed
    ...
    def explain_last_response(self):
        global last_ai_response
        if last_ai_response:
            translation = translate_to_english(last_ai_response, self.loop, self.initial_language)  # Use self.language
...
def on_language_selected(language, lang_window_root, show_language_selection):
    ...
    role_root = tk.Tk()
    role_selection = RoleSelectionWindow(role_root,
                                        lambda role, role_window_root, language=language: on_role_selected(role, role_window_root,
...
if __name__ == "__main__":
    def show_language_selection():
        main_root = tk.Tk()
        lang_selection = LanguageSelectionWindow(main_root,
                                                lambda lang, lang_window_root: on_language_selected(lang, lang_window_root,
                                                                                                     show_language_selection))
        main_root.mainloop()
